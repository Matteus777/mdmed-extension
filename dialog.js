var ACCESS_TOKEN =
  "sl.Bdd1yTa793klkxGB0-688N08EYR0PhPXIGbDSO-06Qy18ZNqAnDgeqNBw--nV5s5nnpWg3skKdmEnyZZT84_7EK7Z1CBsOlNgAKcusZ8xF5Gz7bSXUsmLQd9r_tieLeQ9UQEvHd1";
var REFRESH_TOKEN =
  "IEsPScDQmxkAAAAAAAAAAR3OYJOYqycs7ivq6PCjm5alFeu3eJyGSc7Y1L26YTqz";
...
function observerCallback() {
  if (
    window.location.href.startsWith(
      "https://mdmed.clinic/main/medical-history/view/"
    )
  ) {
    initContentScript();
  }
...
function initContentScript() {
  ...
  async function loadIcons(elements) {
    for (let i = 0; i < elements.length; i++) {
      ...
      wppIcon.style =
        "margin-left:30px;background-color:green;color:white;border:none;border-radius:8px;padding:8px;";
      ...
      wppIcon.onclick = async () => {
        const { laudoBase64 } = await chrome.runtime.sendMessage({
          type: "id",
          data: id,
        });
      ...
      .then((html) => {
        ...
        function checkPhoneNumber(phoneString) {
          if (phoneString.length == 11) {
            const checkedPhone =
              phoneString.substring(0, 2) + phoneString.substring(3);
            return checkedPhone;
          }
          return phoneString;
        }
          ...
          fetch("https://api.dropbox.com/oauth2/token", {
            ...
            .then(async (data) => {
              ...
              for (let i = 0; i < imagens.length; i++) {
                ...
                myHeaders.append(
                  "Dropbox-API-Arg",
                  '{"autorename":true,"mode":"add","mute":false,"path":"/' +
                    FOLDER +
                    "/" +
                    file.name +
                        ...
                        .then((data) => {
                          requestOptions = {
                            ...
                            body: new URLSearchParams({
                              ...
                              Body: `Acesse o link para ter acesso ao seu laudo e imagens ${
                                data.url ?? ", link ja enviado anteriormente."
                              }`,
                            }),
                          };
                          fetch(
                            `https://api.twilio.com/2010-04-01/Accounts/ACb84497dbfd9712a9af350581d87d7ab3/Messages.json`,
                            requestOptions
                          )
                            .then((data) => {
                              data.text();
